...
const JCUKEN_PATTERN = /ф|и|с|в|у|а|п|р|ш|о|л|д|ь|т|щ|з|й|к|ы|е|г|м|ц|ч|н|я|ё|х|ъ|б|ю|э|ж|и/i;
const ERT_PATTERN = /[a-z]/i;

const LOCALE_MACRO_AZERTY = [
  "fr", // French
  "mg", // Malagasy
  "lu", // Luxembourgish
];

const LOCALE_MACRO_JCUKEN = [
  "ab", // Abkhazian
  "ba", // Bashkir
  "be", // Belarusian
  "bg", // Bulgarian
...
): KeyboardLayout
{
  ...
  if (
    LOCALE_MACRO_AZERTY.includes(macro)
        || tag.startsWith("nl-be") // Flemish (Belgium)
  ) return "AZERTY";

  if (
    LOCALE_MACRO_JCUKEN.includes(macro)
  ) return "JCUKEN";

  if (
    LOCALE_MACRO_QWERTZ.includes(macro)
        || tag.startsWith("sr-latn") // Serbian (Latin)
  ) return "QWERTZ";
